...
\begin{equation}
    \vb{E}(\vb{r}, t) = \sum_{k=1}^3 \vb{E}_{\omega_k}(\vb{r}, t) = \frac{1}{2} \sum_{k=1}^3 \large ( \vb{E}_{\omega_k}\large(\vb{r})e^{i\omega_k t} + c.c.)
\end{equation}

Substituting into third-order polarization

\begin{align}
  P^{(3)} & = \frac{3}{4} \varepsilon_0 \chi^{(3)} \left[| E_{\omega_1} |^2 E_{\omega_1} + \cdots\right] & \mathrm{SPM} \\
  & +  \frac{6}{4} \varepsilon_0 \chi^{(3)} \left[(| E_{\omega_2} |^2 + |E_{\omega_3} |^2) E_{\omega_1} + \cdots \right] & \mathrm{XPM} \\
  & +  \frac{1}{4} \varepsilon_0 \chi^{(3)} \left[(E_{\omega_1}^3 e^{i \omega_1 t} + c.c.) + \cdots\right] & \mathrm{THG} \\
  & +  \frac{3}{4} \varepsilon_0 \chi^{(3)} \left[ \frac{1}{2} (E_{\omega_1}^2 E_{\omega_2} e^{i (2 \omega_1 + \omega_2) t} + c.c.) + \cdots \right] & \mathrm{FWM} \\
  & +  \frac{3}{4} \varepsilon_0 \chi^{(3)} \left[ \frac{1}{2} (E_{\omega_1}^2 E^{\ast}_{\omega_2} e^{i (2 \omega_1 - \omega_2) t} + c.c.) + \cdots \right] & \mathrm{FWM} \\ \label{eq:def-dfwm}
  & +  \frac{6}{4} \varepsilon_0 \chi^{(3)} \left[ \frac{1}{2} (E_{\omega_1} E_{\omega_2} E_{\omega_3} e^{i (\omega_1 + \omega_2 + \omega_3) t} + c.c.) + \cdots \right] & \mathrm{FWM} \\
  & +  \frac{6}{4} \varepsilon_0 \chi^{(3)} \left[ \frac{1}{2} (E_{\omega_1} E_{\omega_2} E^{\ast}_{\omega_3} e^{i (\omega_1 + \omega_2 - \omega_3) t} + c.c.) + \cdots \right] & \mathrm{FWM} \label{eq:def-ndfwm}
\end{align}

In above equations, $\cdots$ stands for all possible permutation terms contributed by frequencies $\omega_1, \omega_2, \omega_3$. The abbreviation on the right side are 

\begin{itemize}
    \item SPM, self-phase modulation
    \item XPM, cross-phase modulation
    \item THG, third-harmonic generation
    \item FWM, four wave mixing 
    % \item DFWM, degenerate four wave mixing
\end{itemize}

Another factor to mention is that the coefficient in each $\chi^{(3)}$ process arises from the combination of three frequency components. For instance, on the numerator \autoref{eq:def-dfwm} has the factor 3 but \autoref{eq:def-ndfwm} has the factor 6. This suggest that the interaction of two external field is mathematically two stronger than the single external field case, which is also called as \textit{degenerate four wave mixing} (DFWM).
